...
- **How does K8s work?**
    ...
    1. User using Jupiter need to worry which objects?
        - Deployments, Service, Ingress
    2. NodePort, Service in a Node in Jupiter, Which port should people use to visit the service?
        - Should via NodePort but Jupiter nodes doesn't have NodePorts.
        - Because nodes are actually in private subnet.
    3. Check knowledge about Namespace
        - Isolating groups of resources within a single cluster
    4. About StatefulSets — Where is the stateful info stored?
        - Persisted volume
    5. What does controllers do?
        - CRUD resource objects?
        - Controller manager → Manage controllers;
    6. Will Pod IP change after it is deleted and start a replacement pod?
        - Theoretically yes, but could be the same in minikube.
    7. The actual sequence of what happened after `kubectl create -f` or `kubectl apply -f`
        ...
        Where is controller-manager in this picture?
        
        - 这个线程图是描述create pod 而不是deployment之类的
        - 所以不会有controller manager出现。
    8. Label & Selector are they objects? 
        - No
    9. Why you get a ReplicaSet for free when you `k create -f` a deployment yaml file?
        - Deployment manages ReplicaSet, so it is suggested to use deployment instead of ReplicaSet.
        - Maybe you never need to manipulate ReplicaSet objects, use Deployment instead.
    
    ---
    
    - why we can use [skip ci] to change github repo contents without triggering buildkite build??
